...
# USERNAME
rm -f /usr/bin/user
username=$(curl https://raw.githubusercontent.com/Andyyuda/izin/main/ip | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/user
expx=$(curl https://raw.githubusercontent.com/Andyyuda/izin/main/ip | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
...
Exp1=$(curl https://raw.githubusercontent.com/Andyyuda/izin/main/ip | grep $MYIP | awk '{print $4}')
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
function first_setup(){
    ...
    if [[ $(. /etc/os-release; echo "$ID") == "ubuntu" ]]; then
        echo "Setup Dependencies $(. /etc/os-release; echo "$PRETTY_NAME")"
        rm -f /etc/apt/sources.list.d/*haproxy*
        apt-get update -y
        apt-get install -y --no-install-recommends software-properties-common
        apt-get install -y haproxy
    elif [[ $(. /etc/os-release; echo "$ID") == "debian" ]]; then
        echo "Setup Dependencies For OS Is $(. /etc/os-release; echo "$PRETTY_NAME")"
        apt-get update -y
        apt-get install -y haproxy
    else
...
restart_system() {
    USRSC=$(wget -qO- https://raw.githubusercontent.com/Andyyuda/izin/main/ip | grep $ipsaya | awk '{print $2}')
    EXPSC=$(wget -qO- https://raw.githubusercontent.com/Andyyuda/izin/main/ip | grep $ipsaya | awk '{print $3}')
...
wget raw.githubusercontent.com/Andyyuda/vip/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
echo "[ZiVPN] Setup UDP redirect..."

IFACE=$(ip route get 1.1.1.1 | awk '{print $5; exit}')
echo "Interface: $IFACE"

# hapus rule lama
iptables -t nat -D PREROUTING -i "$IFACE" -p udp --dport 6000:19999 -j REDIRECT --to-ports 5667 2>/dev/null || true

# tambah rule baru
iptables -t nat -I PREROUTING 1 -i "$IFACE" -p udp --dport 6000:19999 -j REDIRECT --to-ports 5667

netfilter-persistent save || true

# install zivpn
wget -O /root/zi.sh https://raw.githubusercontent.com/altunnel/udp-zivpn/main/zi.sh
chmod +x /root/zi.sh
bash /root/zi.sh
